		...
		<div class="main-right">
			...
			<form action="address.ad" id="addressForm">
				<div id="top-menu">
					...
					<div>
						...
						<button type="button" class="button" onclick="location.href='address.ad?code=2&dept=zero&job=zero'">사내주소록</button>
						</c:if>

						<c:if test="${ code == 2 }">
						<div>		
						<button type="button" class="button" onclick="location.href='address.ad'">사외주소록</button>
						</div>
						</c:if>			
				</div>
			...
			<table class="addressList">
				...
				<tr class="list">
					...
					<td onclick="location.href='detailAddress.ad?aNo=${ l.aNo }'">${ l.name }</td>
					<td>${ l.job }</td>
					<td>${ l.email }</td>
					<td>${ l.phone }</td>
					<td>${ l.comName }</td>
				...
				<tr class="list">
					...
					<td onclick="location.href='detailAddress.ad?mNo=${ l.mNo }&code=2'">${ l.mName }</td>
					<td>${ l.jCode }</td>
					<td>${ l.mEmail }</td>
					<td>${ l.mPhone }</td>
					<td>${ l.mBirthDay }</td>
			...
			<div style="text-align: center;">
				...
				<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
					...
					<c:if test="${ pi.currentPage == p }">
						<span style="font-weight: bold; font-size: 18px; color: cornflowerblue;">${ p }</span>&nbsp;&nbsp;
					</c:if>
